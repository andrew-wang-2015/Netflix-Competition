...
def main():
    ...
    with open('small_train.dta','r') as f:
        for line in f:
            ...
            if user not in onlyUserMovie:
                ...
                #rated = dict()
    top10k = []
    with open('TopTenThousandUsers.txt', 'r') as f:
        for line in f:
            top10k.append(int(line))
    
    print("Loaded the top 10000 users")
    assert(len(top10k) == 10000)
            #rated[movie] = rating 
    #moviesRated[prevUser] = rated
    #numRated = dict()
    #for user in onlyUserMovie:
        #numRated[user] = len(onlyUserMovie[user])
    #count = 0
    #sorted_numRated= sorted(numRated.items(), key=operator.itemgetter(1), reverse=True)    
    ...
    print("Generated valid neighbors for all users")
    noNeighbors = 0
    for i in validNeighbors:
        if len(validNeighbors[i]) == 0:
            noNeighbors += 1
    print("Users with no neighbors" + str(noNeighbors))
    print(validNeighbors)
    
    validNeighborsSaved = open('validneighbors_small.txt', 'w')
    for i in validNeighbors:
        validNeighborsSaved.write(str(i) + " ")
        for j in validNeighbors[i]:
            validNeighborsSaved.write(str(j) + " ")
        validNeighborsSaved.write("\n")
